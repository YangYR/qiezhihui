...
$.fn.xb_scroll = function(options)
{
	...
	if(opts.isMaxHeight)
	{
		var sham = $("<div style='position:relative;background:#333;z-index:-1;height:"+elChild.outerHeight()+"px'></div>");
		sham.appendTo(elem);
	}

	var jqScrollBox = $("<div style='position:absolute;width:"+opts.boxWidth+"px;top:"+opts.barOffTop+"px;right:"+opts.barOffRight+"px;bottom:"+opts.barOffBottom+"px;background: rgb(239,239,239);'></div>");
	jqScrollBox.addClass(opts.boxClass);
	var jqScrollBar= $("<div style='position:absolute;width:"+opts.barWidth+"px;top:"+opts.barOffTop+"px;right:"+opts.barOffRight+"px;background: rgb(204,204,204);'></div>");
	...
	jqScrollBar.on("mousedown",function(ev){
		...
		jqScrollBar.css({"background":"rgb(204,204,204)"});
